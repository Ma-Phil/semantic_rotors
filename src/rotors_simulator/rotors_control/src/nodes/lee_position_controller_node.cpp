...
void LeePositionControllerNode::MultiDofJointTrajectoryCallback(
  ...
  // 固定时间间隔，单位：秒
  const ros::Duration fixed_time_interval(1.0); 
...
void LeePositionControllerNode::OdometryCallback(const nav_msgs::OdometryConstPtr& odometry_msg) {
  ...
  // 修改为使用 position 成员
  // ROS_INFO("Current position: %f, %f, %f", odometry.position.x(), odometry.position.y(), odometry.position.z());

  // 打印目标位置
  // const mav_msgs::EigenTrajectoryPoint& current_reference = lee_position_controller_.GetTrajectoryPoint();
  // ROS_INFO("Target position: %f, %f, %f", current_reference.position_W.x(), current_reference.position_W.y(), current_reference.position_W.z());
  ...
  // 打印 ref_rotor_velocities
  // ROS_INFO("ref_rotor_velocities: %f, %f, %f, %f", ref_rotor_velocities[0], ref_rotor_velocities[1], ref_rotor_velocities[2], ref_rotor_velocities[3]);
